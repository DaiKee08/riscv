...
initial begin
    ...
    //
    // Strategy: predict every branch as not taken and continue
    // fetching sequentially (PC+4). The branch outcome is resolved
    // in the Execute stage when PC_ALU_SEL is computed.
    //
    // - Correct prediction (branch not taken): 0 cycle penalty.
    // - Misprediction (branch actually taken): flush the two
    //   wrongly-fetched instructions (in Fetch and Decode stages)
    //   by squashing them to NOPs, then redirect PC to the branch
    //   target. Penalty = 2 cycles.
    //
    // This also handles JAL/JALR (always redirect), which incur
    // the same 2-cycle flush penalty.
    //
    // Compared to the previous always-stall approach (2 cycles per
    // branch regardless), Always-Not-Taken is faster whenever the
    // branch is not taken (0 penalty vs 2 cycles).
    // ============================================================

    // BRANCH_MISPREDICT is asserted in the Execute stage when the
    // branch/jump outcome requires a PC redirect. PC_ALU_SEL is
    // already computed there using forwarded register values.
    wire BRANCH_MISPREDICT = PC_ALU_SEL;
    ...
    //
    // Branch prediction: Always-Not-Taken.
    // PC always advances to PC+4 (no stall on branches).
    // On misprediction (BRANCH_MISPREDICT), PC is redirected to
    // ALU_OUT (the branch target) and the pipeline hazard registers
    // for the wrongly-fetched instruction are squashed to NOP.
    // ------------------------------------------------------------
    ...
    always @(posedge CLK) begin
        if (PC_ALU_SEL == 1) begin
            // Misprediction or unconditional jump: redirect PC to target.
            PC <= ALU_OUT[9:0];
        end
        else begin
            // No branch taken: stall only for load-use hazard.
            if (LOAD_STALL == 1)
                PC <= PC;
            else
                PC <= PC + 4;
        ...
        // Pipeline hazard registers: squash wrongly-fetched instruction
        // on misprediction; otherwise decode the incoming instruction.
        if (BRANCH_MISPREDICT) begin
            // Flush: the instruction currently entering Decode is invalid.
            R1_PIPELINE[DECODE]   <= 0;
    ...
    // 2. Decode
    //
    // On misprediction: squash the instruction currently in Decode
    // (branch+4) before it enters Execute. Replace with NOP.
    // Also clear the Execute-slot pipeline hazard registers so
    // forwarding logic does not see a false dependency.
    // ------------------------------------------------------------

    always @(posedge CLK) begin
        if (LOAD_STALL == 1) begin
            // Hold Decode register; insert bubble into Execute.
            INSTRUCTION_DECODE_2 <= INSTRUCTION_DECODE_2;
            PC_DECODE_2 <= PC_DECODE_2;
        end
        else if (BRANCH_MISPREDICT) begin
            // Flush: squash the speculatively-fetched instruction.
            INSTRUCTION_DECODE_2 <= 32'h00000013; // NOP
            PC_DECODE_2 <= PC_DECODE_2;
        end
        else begin
        ...
        // Update Execute-slot pipeline hazard registers.
        ...
        // Overwrite with NOP metadata for load-use bubble or mispredict flush.
        if (LOAD_STALL == 1 || BRANCH_MISPREDICT) begin
            R1_PIPELINE[EXECUTE]   <= 0;
            R2_PIPELINE[EXECUTE]   <= 0;
    ...
    // 3. Execute
    //
    // On misprediction: the instruction moving from Decode into
    // Execute is the wrongly-fetched branch+4. Squash it to NOP.
    // The branch itself continues normally to Memory/Writeback
    // (it does not write a register, so it is harmless).
    // ------------------------------------------------------------

    always @(posedge CLK) begin
        if (LOAD_STALL == 1) begin
            // Insert load-use bubble.
            INSTRUCTION_EXECUTE_3 <= 32'h00000013;
            PC_EXECUTE_3 <= PC_EXECUTE_3;
        end
        else if (BRANCH_MISPREDICT) begin
            // Flush: squash the instruction that was in Decode.
            INSTRUCTION_EXECUTE_3 <= 32'h00000013; // NOP
        ...
        // Move Execute instruction's metadata into Memory slot.
        // (This always reflects the instruction currently in Execute,
        //  which is the branch itself — correct behaviour.)
